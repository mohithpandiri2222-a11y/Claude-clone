...
use runtime::{TokenUsage, Session, ProjectContext, ConfigLoader, MessageRole, ContentBlock, PermissionMode};
use crate::git::{parse_git_status_metadata, find_git_root};
...
pub fn status_context(
    ...
    let project_context = ProjectContext::discover_with_git(&cwd, &crate::current_date_string())?;
...
pub fn render_config_report(section: Option<&str>) -> Result<String, Box<dyn std::error::Error>> {
    Ok(format!("Config report for section: {}", section.unwrap_or("all")))
}

pub fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
    Ok("Memory report: [redacted]".to_string())
}
