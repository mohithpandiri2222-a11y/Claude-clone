...
use api::{AuthSource, resolve_startup_auth_source, AnthropicClient};
use runtime::{
    ConfigLoader, clear_oauth_credentials, generate_pkce_pair, generate_state,
    OAuthConfig, OAuthAuthorizationRequest, OAuthTokenExchangeRequest,
    save_oauth_credentials, parse_oauth_callback_request_target,
};
...
pub fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let runtime_tokio = tokio::runtime::Runtime::new()?;
    let token_set = runtime_tokio.block_on(client.exchange_oauth_code(oauth, &exchange_request))?;
